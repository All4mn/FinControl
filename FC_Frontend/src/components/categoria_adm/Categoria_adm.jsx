...
import { Utensils, BusFront, House, Cross, Store, Trash2, Tag, Pencil, Check, CircleDollarSign } from "lucide-react";
...
const iconGroupMap = {
  Utensils: [
    "alimentacao", "comida", "restaurante", "almoco", "jantar",
    "cafe", "lanche", "fastfood", "padaria", "gourmet",
    "refeicao", "marmita", "pizzaria", "churrasco",
  ],
  BusFront: [
    "lazer", "viagem", "transporte", "onibus", "passagem",
    "cinema", "show", "teatro", "festa", "balada",
    "passeio", "entretenimento", "ferias", "turismo",
    "combustivel", "uber",
  ],
  House: [
    "moradia", "casa", "aluguel", "condominio", "luz",
    "agua", "internet", "gas", "energia", "reforma",
    "moveis", "iptu", "imovel",
  ],
  Cross: [
    "saude", "medico", "hospital", "farmacia", "remedio",
    "consulta", "dentista", "exame", "clinica", "psicologo",
    "convenio", "seguro",
  ],
  Store: [
    "mercado", "supermercado", "compras", "hipermercado",
    "mercearia", "feira", "sacolao", "atacado", "varejo",
  ],
  CircleDollarSign: [
    "salario", "renda", "receita", "bonus", "dividendos",
    "pix", "transferencia", "investimento", "lucro", "ganho",
  ],
};
  ...
  const handleSaveEdit = async (e, id) => {
    ...
    try {
      ...
      setCategorias((prev) =>
        prev.map((cat) =>
          cat.id === id
            ? { ...cat, label: nome, icon: encontrarIcone(iconGroupMap, nome) }
            : cat,
        ),
      );
      if (selected?.id === id) {
