      ...
      <div className={styles.table_container}>
        {/* {logs.logs.map((log) => (
          ...
          {logs.logs.length > 0 ? (
              ...
              <tbody>
                {logs.logs.map((log) => (
                  <tr key={log.id_log} className={styles.linha}>
                    ...
                    <td>{log.valor}</td>
                    <td>
                      {log.valor_antes !== null ? log.valor_antes : "N/A"}
                    </td>
                    <td>{log.entrada ? "Sim" : "Não"}</td>
                    <td>
                      {log.entrada_antes !== null
                        ? log.entrada_antes
                          ? "Sim"
                          : "Não"
                        : "N/A"}
                    ...
                    <td>
                      {log.data_transacao
                        ? dayjs(log.data_transacao).format(
                            "DD/MM/YYYY HH:mm:ss",
                          )
                        : "N/A"}
                    </td>
                    <td>
                      {log.data_antes
                        ? dayjs(log.data_antes).format("DD/MM/YYYY HH:mm:ss")
                        : "N/A"}
                    </td>
                    <td>
                      {log.data_log
                        ? dayjs(log.data_log).format("DD/MM/YYYY HH:mm:ss")
                        : "N/A"}
                    </td>
                  </tr>
                ))}
            ...
            <>
              <h3>Carregando...</h3>
              <button onClick={()=>console.log(logs)}>sdasd</button>
            </>
          )}
          {/* <thead>
            <tr>
          ...
          <tbody>
            {logs.logs.map((log) => (
              <tr key={log.id_log} className={styles.linha}>
                ...
                <td>
                  {log.data_transacao
                    ? dayjs(log.data_transacao).format("DD/MM/YYYY HH:mm:ss")
                    : "N/A"}
                </td>
                <td>
                  {log.data_antes
